...
class TestReduce(unittest.TestCase):
    def test_copy_value(self):
        node = FTL.Message(id=FTL.Identifier("key"), value=COPY("path", "key"))

        self.assertEqual(fold(get_source, node, ()), (("path", "key"),))

    def test_copy_traits(self):
        node = FTL.Message(
            id=FTL.Identifier("key"),
            attributes=[
                FTL.Attribute(FTL.Identifier("trait1"), value=COPY("path1", "key1")),
                FTL.Attribute(FTL.Identifier("trait2"), value=COPY("path2", "key2")),
            ],
        )

        self.assertEqual(
            fold(get_source, node, ()), (("path1", "key1"), ("path2", "key2"))
        )

    def test_copy_concat(self):
        node = FTL.Message(
            FTL.Identifier("hello"),
            value=CONCAT(COPY("path1", "key1"), COPY("path2", "key2")),
        )
    ...
    def test_copy_in_replace(self):
        node = FTL.Message(
            FTL.Identifier("hello"),
            value=REPLACE("path1", "key1", {"foo": COPY("path2", "key2")}),
        )
