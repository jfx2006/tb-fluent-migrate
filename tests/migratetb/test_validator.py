...
@mock.patch("fluent.migratetb.validator.MigrateAnalyzer", autospec=True)
class TestValidator_inspect_migrate(unittest.TestCase):
    def test_bad_args(self, analyzer):
        ...
        v = validator.Validator("def migrate(*args, **kwargs):\n pass", "bug_1.py")
        with self.assertRaises(validator.MigrateNotFoundException):
            v.inspect_migrate(v.ast.body[0], {})
    ...
    def test_ctx_var(self, Analyzer):
        ...
        self.assertDictEqual(rv, {"issues": [], "references": []})
...
class TestMigrateAnalyzer_Call(unittest.TestCase):
    def test_other(self, call_ctx, call_helpers_transforms_from, call_transform):
    ...
    def test_ctx(self, call_ctx, call_helpers_transforms_from, call_transform):
        m = ast.parse("foo.bar()")
        v = validator.MigrateAnalyzer("foo", {})
        v.visit(m)
        self.assertListEqual(v.issues, [])
    ...
    def test_transform(self, call_ctx, call_helpers_transforms_from, call_transform):
        m = ast.parse("COPY()")
        v = validator.MigrateAnalyzer("foo", {"COPY": "fluent.migratetb.COPY"})
        v.visit(m)
        self.assertListEqual(v.issues, [])
        call_ctx.assert_not_called()
    ...
    def test_helpers(self, call_ctx, call_helpers_transforms_from, call_transform):
        m = ast.parse("helpers.transforms_from()")
        v = validator.MigrateAnalyzer("foo", {"helpers": "fluent.migratetb.helpers"})
        v.visit(m)
        call_ctx.assert_not_called()
        call_transform.assert_not_called()
...
class TestMigrateAnalyzer_ctx(unittest.TestCase):
    def test_bad_api(self, call_add_transforms):
    ...
    def test_add_transforms(self, call_add_transforms):
...
class TestMigrateAnalyzer_add_transforms(unittest.TestCase):
    ...
    def test_paths(self):
        ...
        v.references.clear()
        call = (
            ast.parse('foo.add_transforms("some/fluent.ftl", target, [])').body[0].value
        )
        v.call_add_transforms(call)
        self.assertListEqual(v.issues, [])
        self.assertSetEqual(v.references, {"some/fluent.ftl"})
        v.references.clear()
        call = ast.parse('foo.add_transforms(src, "some/fluent.ftl", [])').body[0].value
        v.call_add_transforms(call)
        self.assertListEqual(v.issues, [])
        self.assertSetEqual(v.references, {"some/fluent.ftl"})
        v.references.clear()
        call = (
            ast.parse('foo.add_transforms("some/fluent.ftl", "some/fluent.ftl", [])')
            .body[0]
            .value
        )
        v.call_add_transforms(call)
    ...
    def test_different_paths(self):
        ...
        v = validator.MigrateAnalyzer("foo", {})
        call = ast.parse('foo.add_transforms("a.ftl", "b.ftl", [])').body[0].value
        v.call_add_transforms(call)
...
class TestTransformsInspector(unittest.TestCase):
    ...
    def test_copy_with_issue(self):
        ...
        self.assertListEqual(
            ti.issues, ['Source "./foo/bar.dtd" needs to be a normalized path']
        )
    ...
    def test_copy_pattern_with_issue(self):
        ...
        ti.visit(node)
        self.assertListEqual(
            ti.issues, ['Source "./foo/bar.ftl" needs to be a normalized path']
        )
